const originalUrl = '/src/ui/roadRunnerRuntimeV6.js?v=world-map-source';
...
function patchRuntime(source) {
  ...
  return { patched, routesPatched: routes.patched, hazardPatched: hazard.patched, wearPatched, livePatched, ghostPatched: ghost.patched };
}
...
async function loadRuntime() {
  ...
  console.info('[365 Racer] World maps V6 runtime loaded', result);
}

loadRuntime().catch((error) => {
  console.error('[365 Racer] World maps runtime patch failed. Loading original V6.', error);
  import(originalUrl);
});
