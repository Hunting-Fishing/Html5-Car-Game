...
function normalizeUpgrades(upgrades = {}) {
  ...
  const result = {};
  Object.keys(defaults).forEach((key) => { result[key] = Math.max(1, Math.min(UPGRADES[key].max, Number(upgrades[key] || defaults[key]))); });
  return result;
}
function loadSave() {
  try {
    const parsed = JSON.parse(localStorage.getItem(SAVE_KEY) || '{}');
    return { coins: parsed.coins || 0, parts: parsed.parts || 0, bestDistance: parsed.bestDistance || 0, bestTrail: Array.isArray(parsed.bestTrail) ? parsed.bestTrail : [], upgrades: normalizeUpgrades(parsed.upgrades) };
  } catch { return { coins: 0, parts: 0, bestDistance: 0, bestTrail: [], upgrades: defaultUpgrades() }; }
}
...
function vehicleStats() { const u = normalizeUpgrades(saveData.upgrades); return { acceleration: 82 + u.engine * 8, topSpeed: 130 + u.tires * 6, drag: Math.max(3.5, 11 - u.tires * 0.35), startEnergy: 95 + u.energyTank * 18, energyUse: Math.max(1.9, 3.9 - u.energyTank * 0.08), suspension: 0.08 + u.suspension * 0.012 }; }
...
function shellHtml() {
  return `
    <section class="card roadRunnerShell">
      <div class="roadRunnerHeader"><h2>365 Hill Route - Stable Canvas Mode</h2><p>Race mode rebuilt without Phaser. Hold GAS to drive, BRAKE to slow. Track, cars, pickups, and controls render immediately.</p></div>
      ...
      <section class="roadRunnerPanel"><div class="roadRunnerPanelTitle"><h3>Ghost Mode</h3><button class="btn primary" onclick="window.restartHillRoute?.()">Restart</button></div><div class="roadRunnerModeGrid">${Object.entries(GHOST_MODES).map(([key, value]) => `<button class="btn ${key === activeGhostMode ? 'primary' : 'ghost'}" onclick="window.setHillGhosts?.('${key}')">${value.label}</button>`).join('')}</div></section>
...
function updateHud() { if (!game) return; const energyPercent = Math.max(0, Math.min(100, game.energy / game.stats.startEnergy * 100)); setText('[data-rr-distance]', `${Math.floor(game.distanceM)}m`); setText('[data-rr-fuel]', `${Math.floor(energyPercent)}%`); setText('[data-rr-coins]', fmt(saveData.coins)); setText('[data-rr-parts]', fmt(saveData.parts)); setText('[data-rr-best]', `${fmt(saveData.bestDistance)}m`); setText('[data-rr-route]', ROUTES[activeRoute].label); setText('[data-rr-mode]', GHOST_MODES[activeGhostMode].label); renderGaragePanel(); }
function setText(selector, value) { const element = document.querySelector(selector); if (element) element.textContent = value; }
function renderGaragePanel() { const panel = document.querySelector('[data-road-runner-garage]'); if (!panel) return; panel.innerHTML = `<div class="roadRunnerPanelTitle"><h3>Garage Upgrades</h3><span class="roadRunnerNotice">Run, earn, upgrade, go farther.</span></div><div class="roadRunnerWalletRow"><div class="roadRunnerWalletPill">Coins: ${fmt(saveData.coins)}</div><div class="roadRunnerWalletPill">Parts: ${fmt(saveData.parts)}</div></div><div class="roadRunnerGarageGrid">${Object.entries(UPGRADES).map(([key, item]) => upgradeCard(key, item)).join('')}</div>`; }
...
function drawPickups(width, height, route, cameraX) { for (const pickup of game.pickups) { if (pickup.collected) continue; const x = pickup.x - cameraX; if (x < -40 || x > width + 40) continue; const y = routeY(route, pickup.x, height) - 34; const img = images[pickup.type]; if (img?.ready) ctx.drawImage(img, x - 15, y - 15, 30, 30); else { ctx.fillStyle = pickup.type === 'energy' ? '#38bdf8' : pickup.type === 'parts' ? '#a78bfa' : '#facc15'; ctx.beginPath(); ctx.arc(x, y, 13, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#06131d'; ctx.font = '900 12px Arial'; ctx.textAlign = 'center'; ctx.fillText(pickup.type === 'energy' ? 'E' : pickup.type === 'parts' ? 'P' : 'C', x, y + 4); } } }
function drawGhosts(width, height, route, cameraX) { const mode = GHOST_MODES[activeGhostMode]; const ghostKeys = ['ghostA', 'ghostB', 'ghostC']; for (let i = 0; i < mode.count; i++) { const p = ghostPoint(i, height); const x = p.x - cameraX; if (x < -100 || x > width + 120) continue; drawCar(x, p.y, routeAngle(route, p.x, height), images[ghostKeys[i]], `Ghost ${i + 1}`, 0.50, game.ghostColors[i]); } }
...
function drawTopDebug() { ctx.fillStyle = 'rgba(4,17,29,0.70)'; roundRect(12, 42, 160, 30, 14); ctx.fill(); ctx.fillStyle = '#fff'; ctx.font = '900 11px Arial'; ctx.textAlign = 'left'; ctx.fillText(`Speed ${Math.floor(game.speed)}  Energy ${Math.floor(game.energy)}`, 24, 62); }
function drawFinishMessage(width, height) { ctx.fillStyle = 'rgba(4,17,29,0.88)'; roundRect(width / 2 - 150, height / 2 - 52, 300, 104, 18); ctx.fill(); ctx.fillStyle = '#fff'; ctx.font = '900 16px Arial'; ctx.textAlign = 'center'; ctx.fillText(game.message, width / 2, height / 2 - 8); ctx.font = '800 12px Arial'; ctx.fillText('Tap Restart to run again.', width / 2, height / 2 + 18); }
...
function collectVisiblePickups() { for (const pickup of game.pickups) { if (pickup.collected) continue; if (Math.abs(game.x - pickup.x) > 36) continue; pickup.collected = true; if (pickup.type === 'energy') game.energy = Math.min(game.stats.startEnergy + 24, game.energy + 18); if (pickup.type === 'parts') game.parts += 1; if (pickup.type === 'coin') game.coins += Math.round(4 * game.route.reward); } }
...
function mountRoadRunner(force = false) { const screen = document.querySelector('#screen-race.active'); if (!screen) return; if (force || screen !== mountedScreen || !screen.querySelector('.roadRunnerShell')) { saveData = loadSave(); screen.innerHTML = shellHtml(); mountedScreen = screen; canvas = screen.querySelector('#roadRunnerCanvas'); ctx = canvas.getContext('2d'); bindControls(); resizeCanvas(); resetRun(); renderGaragePanel(); draw(); } if (!frameId) { lastTime = performance.now(); frameId = requestAnimationFrame(loop); } }
