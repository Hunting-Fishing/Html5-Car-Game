...
import { fmt, costToText, canAfford, addCurrency, addXp, labelCurrency } from './systems/economySystem.js';
...
function renderHub() {
  ...
  return `
    ...
    <section class="card">
      <div class="cardTitle"><div><h3>Reward Feed</h3><p>Recent rewards, fixes, upgrades, and unlocks.</p></div></div>
      ${state.log.slice(0, 8).map((line) => `<div class="logLine">${line}</div>`).join('') || `<div class="logLine">No rewards yet. Collect, merge, or upgrade to start the feed.</div>`}
    </section>
  `;
}
